...
ConvexMPC::ConvexMPC(const Eigen::Ref<const Eigen::Vector<double, MPC_STATE_DIM>>& state_weight,
    ...
    for (int mpc_step = 0; mpc_step < MPC_HORIZON; mpc_step++) {
        Q_.block<MPC_STATE_DIM, MPC_STATE_DIM>(mpc_step * MPC_STATE_DIM, mpc_step * MPC_STATE_DIM) = state_weight.asDiagonal();
        R_.block<MPC_INPUT_DIM, MPC_INPUT_DIM>(mpc_step * MPC_INPUT_DIM, mpc_step * MPC_INPUT_DIM) = input_weight.asDiagonal();
    }

    linear_constraints_.resize(MPC_CONSTRAINT_DIM * MPC_HORIZON, MPC_INPUT_DIM * MPC_HORIZON);
    int rows = constraint_coefficient.rows();
    int cols = constraint_coefficient.rows();
