...
## Local modifications (this repository)

This copy was vendored from
[mujoco_menagerie](https://github.com/google-deepmind/mujoco_menagerie)
(commit `da76818`) with the following changes for the Convex-MPC controller:

1. Position-servo actuators replaced with direct torque (`motor`) actuators.
2. Foot sites `LF_FOOT`, `RF_FOOT`, `LH_FOOT`, `RH_FOOT` added at the foot
   sphere centers (used for foot kinematics/Jacobians).
3. A `standing` keyframe added (taken from the menagerie MJX variant).
